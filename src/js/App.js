...
class App extends React.Component {
    ...
    fetchData() {
        ...
        fetch('https://randomuser.me/api/?results=50&nat=us,dk,fr,gb')
            ...
            .then(parsedJSON => parsedJSON.results.map(user => ({name: `${user.name.first} ${user.name.last}`, username: `${user.login.username}`, email: `${user.email}`, location: `${user.location.street} ${user.location.city}`})))
            .then(contacts => this.setState({contacts, isLoading: false}))
            .catch(error => console.log('pasing failed', error))
    }
                    ...
                    <div className="panel-group">
                        {!isLoading && contacts.length > 0
                            ? contacts.map(contact => {
                                ...
                                return <Collapsible key={contact.username} title={name}>
